...
async function main() {
    ...
    const base64Tool = await prisma.tool.upsert({
        ...
        create: {
            ...
            slug: 'base64',
            description: 'Encode and decode text and files to/from Base64 format with URL-safe options',
    ...
    const markdownTool = await prisma.tool.upsert({
        ...
        create: {
            ...
            description: 'Convert Markdown to professional PDFs with syntax highlighting',
    ...
    console.log(`Created tools: ${[base64Tool, hashTool, faviconTool, markdownTool].map(t => t.name).join(', ')}`);
    console.log(`Created tags: ${[encodingTag, securityTag, designTag, documentTag].map(t => t.name).join(', ')}`);
}
