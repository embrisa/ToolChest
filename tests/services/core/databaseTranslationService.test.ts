...
describe("DatabaseTranslationService - FULL Integration Tests", () => {
  ...
  describe("translateTool - Core Translation Functionality", () => {
    it("should translate a tool to English using real translation files", async () => {
      ...
      const result = await DatabaseTranslationService.translateTool(
        testTool,
        "en",
      );
  ...
  describe("translateTools - Batch Translation", () => {
    it("should translate multiple tools efficiently", async () => {
      ...
      const result = await DatabaseTranslationService.translateTools(
        testTools,
        "en",
      );
  ...
  describe("translateTag - Tag Translation", () => {
    it("should translate a tag to English", async () => {
      ...
      const result = await DatabaseTranslationService.translateTag(
        testTag,
        "en",
      );
  ...
  describe("translateTags - Batch Tag Translation", () => {
    it("should translate multiple tags", async () => {
      ...
      const result = await DatabaseTranslationService.translateTags(
        testTags,
        "en",
      );
        ...
        createTestTool({
          ...
          nameKey:
            i % 4 === 0
              ? "tools.base64.name"
              : i % 4 === 1
                ? "tools.hash-generator.name"
                : i % 4 === 2
                  ? "tools.favicon-generator.name"
                  : "tools.markdown-to-pdf.name",
          descriptionKey:
            i % 4 === 0
              ? "tools.base64.description"
              : i % 4 === 1
                ? "tools.hash-generator.description"
                : i % 4 === 2
                  ? "tools.favicon-generator.description"
                  : "tools.markdown-to-pdf.description",
        }),
      );
    ...
    it("should handle translation of 100 tags efficiently", async () => {
      ...
      const largeTagsArray = Array.from({ length: 100 }, (_, i) =>
        createTestTag({
          ...
          descriptionKey:
            i % 4 === 0
              ? "tags.encoding.description"
              : i % 4 === 1
                ? "tags.security.description"
                : i % 4 === 2
                  ? "tags.design.description"
  ...
  describe("Error Handling and Edge Cases", () => {
    ...
    it("should handle null/undefined locale gracefully", async () => {
      ...
      const result1 = await DatabaseTranslationService.translateTool(
        testTool,
        null,
      );
      const result2 = await DatabaseTranslationService.translateTool(
        testTool,
        undefined,
      );
  ...
  describe("Translation Key Utilities", () => {
    it("should generate correct tool name keys", () => {
      expect(DatabaseTranslationService.getToolNameKey("base64")).toBe(
        "tools.base64.name",
      );
      expect(DatabaseTranslationService.getToolNameKey("hash-generator")).toBe(
        "tools.hash-generator.name",
      );
      expect(
        DatabaseTranslationService.getToolNameKey("favicon-generator"),
      ).toBe("tools.favicon-generator.name");
    });

    it("should generate correct tool description keys", () => {
      expect(DatabaseTranslationService.getToolDescriptionKey("base64")).toBe(
        "tools.base64.description",
      );
      expect(
        DatabaseTranslationService.getToolDescriptionKey("hash-generator"),
      ).toBe("tools.hash-generator.description");
    });

    it("should generate correct tag name keys", () => {
      expect(DatabaseTranslationService.getTagNameKey("encoding")).toBe(
        "tags.encoding.name",
      );
      expect(DatabaseTranslationService.getTagNameKey("security")).toBe(
        "tags.security.name",
      );
    });

    it("should generate correct tag description keys", () => {
      expect(DatabaseTranslationService.getTagDescriptionKey("encoding")).toBe(
        "tags.encoding.description",
      );
      expect(DatabaseTranslationService.getTagDescriptionKey("security")).toBe(
        "tags.security.description",
      );
    });
    ...
    it("should handle special characters in keys", () => {
      expect(
        DatabaseTranslationService.getToolNameKey("test-tool-with-dashes"),
      ).toBe("tools.test-tool-with-dashes.name");
      expect(
        DatabaseTranslationService.getTagNameKey("test_tag_with_underscores"),
      ).toBe("tags.test_tag_with_underscores.name");
    });
  });
  ...
  describe("Hook-like Functions", () => {
    it("should provide getTranslatedTools function", async () => {
      const { getTranslatedTools } = await import(
        "@/services/core/databaseTranslationService"
      );
    ...
    it("should provide getTranslatedTags function", async () => {
      const { getTranslatedTags } = await import(
        "@/services/core/databaseTranslationService"
      );
  ...
  describe("Service Structure Validation", () => {
    it("should have all required static methods", () => {
      ...
      expect(typeof DatabaseTranslationService.getToolDescriptionKey).toBe(
        "function",
      );
      expect(typeof DatabaseTranslationService.getTagNameKey).toBe("function");
      expect(typeof DatabaseTranslationService.getTagDescriptionKey).toBe(
        "function",
      );
      expect(typeof DatabaseTranslationService.generateToolRecord).toBe(
        "function",
      );
      expect(typeof DatabaseTranslationService.generateTagRecord).toBe(
        "function",
      );
    });
    ...
    it("should validate translation key extraction logic", () => {
      ...
      expect(DatabaseTranslationService.getToolNameKey(toolKey)).toBe(
        expectedNameKey,
      );
      expect(DatabaseTranslationService.getToolDescriptionKey(toolKey)).toBe(
      ...
      expect(DatabaseTranslationService.getTagNameKey(tagKey)).toBe(
        expectedTagNameKey,
      );
      expect(DatabaseTranslationService.getTagDescriptionKey(tagKey)).toBe(
        expectedTagDescriptionKey,
      );
    });
  });
});
