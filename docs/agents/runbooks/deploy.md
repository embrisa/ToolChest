...
4) For non-Docker pipelines, run `npm run deploy:prod` after build to apply migrations + seed, or use the CI hook `npm run ci:postdeploy` (calls migrate deploy + seed).
