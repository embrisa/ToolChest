...
const mappingsByDir = SCHEMA_MAP.reduce<Record<string, MappingEntry[]>>((acc, m) => {
...
function getObjectByPath(obj: any, pathStr: string): any {
    ...
    return pathStr.split(".").reduce((res, key) => (res ? res[key] : undefined), obj);
}
...
async function qaTranslations(): Promise<void> {
    ...
    const modulePaths = await glob("messages/**/", { ignore: "messages/node_modules/**" });

    let totalErrors = 0;
    let filesWithErrors = 0;

    for (const modulePath of modulePaths) {
        const relativeDir = modulePath.replace(/^messages\//, "").replace(/\/$/, "");
    ...
    console.log("\n----------------------------------");
    if (totalErrors === 0) {
        console.log("✅ QA checks passed. All translation files are consistent and valid.");
        process.exit(0);
    } else {
        console.error(`🚨 QA checks failed. Found ${totalErrors} issue(s) in ${filesWithErrors} file(s).`);
        process.exit(1);
    }
}
