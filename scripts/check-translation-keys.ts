...
async function collectEnglishKeys(): Promise<Set<string>> {
    const files = await glob("messages/**/en.json", { ignore: "messages/node_modules/**" });
...
async function collectUsedKeys(): Promise<Map<string, Set<string>>> {
    const translationCallRegex = /[a-zA-Z_$][a-zA-Z0-9_$]*\(\s*(["'`])([^"'`\n]+?)\1/g;
    const result = new Map<string, Set<string>>();
    for (const pattern of SRC_GLOBS) {
        const files = await glob(pattern, { ignore: ["**/*.test.*", "**/__tests__/**", "node_modules/**", ".next/**"] });
        for (const file of files) {
...
async function main() {
    ...
    if (missing.length === 0) {
        console.log("✅ All translation keys referenced in code exist in English messages");
        return;
    }

    console.error("\n❌ Missing translation keys referenced in code:");
    for (const entry of missing) {
        console.error(`  [${path.relative(process.cwd(), entry.file)}] ${entry.key}`);
    }
