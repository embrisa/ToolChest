...
export async function GET(request: NextRequest) {
  try {
    ...
    const filters: RelationshipFilters = {
      ...
      toolIds:
        searchParams.get("toolIds")?.split(",").filter(Boolean) || undefined,
      tagIds:
        searchParams.get("tagIds")?.split(",").filter(Boolean) || undefined,
      toolIsActive:
        searchParams.get("toolIsActive") === "true"
          ? true
          : searchParams.get("toolIsActive") === "false"
            ? false
            : undefined,
    };
    ...
    const sortOptions: RelationshipSortOptions = {
      field:
        (searchParams.get("sortField") as RelationshipSortOptions["field"]) ||
        "toolName",
