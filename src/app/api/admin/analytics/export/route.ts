...
function checkExportRateLimit(clientIP: string): boolean {
  ...
  if (!current || now > current.resetTime) {
    exportCounts.set(clientIP, {
      count: 1,
      resetTime: now + EXPORT_RATE_WINDOW,
    });
    return true;
  }
...
export async function POST(request: NextRequest) {
  try {
    ...
    const {
      exportOptions,
      filter,
    }: { exportOptions: ExportOptions; filter?: AnalyticsFilter } = body;

    // Validate export options
    if (
      !exportOptions.format ||
      !["csv", "pdf", "json"].includes(exportOptions.format)
    ) {
      return NextResponse.json(
        {
          success: false,
    ...
    if (cached && isValidExportCache(cached)) {
      ...
      const remaining = current
        ? Math.max(0, EXPORT_RATE_LIMIT - current.count)
        : EXPORT_RATE_LIMIT - 1;
    ...
    if (exportOptions.format === "csv") {
      const csvData = convertToEnhancedCSV(
        exportData,
        includeCharts,
        includeRawData,
      );
    ...
    if (exportOptions.format === "pdf") {
      ...
      const pdfData = {
        title: "tool-chest Analytics Report",
...
function convertToEnhancedCSV(
  ...
  if (exportData.systemMetrics) {
    ...
    if (metrics.memoryUsage) {
      ...
      sections.push(
        `Memory Usage,"${memUsage.toFixed(1)}%","${memUsage > 80 ? "Warning" : "Normal"}"`,
      );
    }
    if (metrics.apiResponseTimes) {
      sections.push(
        `API Response Time,"${metrics.apiResponseTimes.average}ms","${metrics.apiResponseTimes.average > 1000 ? "Slow" : "Normal"}"`,
      );
    }
    if (metrics.errorRates) {
      sections.push(
        `Error Rate,"${metrics.errorRates.overall}%","${metrics.errorRates.overall > 5 ? "High" : "Normal"}"`,
      );
    }
    sections.push("");
  }
