...
export async function GET(request: NextRequest) {
    try {
        ...
        const cacheParams = { toolId, startDate, endDate, aggregation, limit, offset, orderBy, orderDirection };
        ...
        const summary = {
            ...
            dateRange: {
                start: usageData.length > 0 ? usageData[usageData.length - 1].timestamp : null,
...
function aggregateUsageData(usageData: any[], aggregation: string): any[] {
    ...
    return Array.from(aggregated.values()).map(agg => ({
        ...agg,
        averageProcessingTime: agg.totalUsage > 0 ? agg.totalProcessingTime / agg.totalUsage : 0,
        averageInputSize: agg.totalUsage > 0 ? agg.averageInputSize / agg.totalUsage : 0,
        averageOutputSize: agg.totalUsage > 0 ? agg.averageOutputSize / agg.totalUsage : 0,
        uniqueToolsCount: agg.uniqueTools.size,
        successRate: agg.totalUsage > 0 ? (agg.successfulUsage / agg.totalUsage) * 100 : 0,
