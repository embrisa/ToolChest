...
export async function GET(request: NextRequest) {
    try {
        ...
        const serviceChecks = await Promise.allSettled([
            ...
            analyticsService.getAnalyticsSummary().then(() => ({ service: 'analytics', status: 'healthy' })),

            // Test system metrics collection
            analyticsService.getCurrentMetrics().then(() => ({ service: 'metrics', status: 'healthy' })),

            // Test chart generation
            analyticsService.generateCharts().then(() => ({ service: 'charts', status: 'healthy' })),

            // Test export functionality
            analyticsService.exportAnalytics({ format: 'json', includeCharts: false, includeRawData: false }).then(() => ({ service: 'export', status: 'healthy' })),
        ]);
