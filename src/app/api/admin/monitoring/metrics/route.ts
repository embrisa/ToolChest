...
export async function GET(request: NextRequest) {
    try {
        ...
        if (!checkRateLimit(clientIP)) {
            const current = requestCounts.get(clientIP);
            const retryAfter = current ? Math.ceil((current.resetTime - Date.now()) / 1000) : 60;
        ...
        if (currentOnly) {
            ...
            metricsData = {
                current: filteredMetrics.length > 0 ? filteredMetrics[0] : await analyticsService.getCurrentMetrics(),
        ...
        const current = requestCounts.get(clientIP);
        const remaining = current ? Math.max(0, RATE_LIMIT - current.count) : RATE_LIMIT - 1;
...
function calculateMetricsSummary(metrics: any[]): any {
    ...
    const averages = {
        ...
        memoryUsagePercentage: metrics.reduce((sum, m) => sum + m.memoryUsage.percentage, 0) / metrics.length,
        activeConnections: metrics.reduce((sum, m) => sum + m.activeConnections, 0) / metrics.length,
        requestsPerMinute: metrics.reduce((sum, m) => sum + m.requestsPerMinute, 0) / metrics.length,
        cpuUsage: metrics.reduce((sum, m) => sum + m.cpuUsage, 0) / metrics.length,
        diskUsage: metrics.reduce((sum, m) => sum + m.diskUsage, 0) / metrics.length,
    };
    ...
    const timeRange = {
        ...
        duration: new Date(metrics[0].timestamp).getTime() - new Date(metrics[metrics.length - 1].timestamp).getTime(),
    };
...
function calculateTrends(metrics: any[]): any {
    ...
    return {
        apiResponseTime: calculateTrend(first.apiResponseTime, last.apiResponseTime),
        errorRate: calculateTrend(first.errorRate, last.errorRate),
        memoryUsage: calculateTrend(first.memoryUsage.percentage, last.memoryUsage.percentage),
