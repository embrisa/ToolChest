      ...
      case "delete":
        for (const tagId of tagIds) {
          try {
            ...
            const warning =
              await adminTagService.checkDeleteRelationshipWarnings(tagId);
            ...
            errors.push(
              `Tag ${tagId}: ${error instanceof Error ? error.message : "Unknown error"}`,
            );
          }
        }
        break;
        ...
        for (const tagId of tagIds) {
          try {
            ...
            if (tag) {
              ...
              const isAvailable = await adminTagService.checkSlugAvailable(
                newSlug,
                tagId,
              );
              ...
              if (isAvailable) {
                ...
                errors.push(
                  `Tag ${tagId}: Generated slug '${newSlug}' is already in use`,
                );
              }
            }
          } catch (error) {
