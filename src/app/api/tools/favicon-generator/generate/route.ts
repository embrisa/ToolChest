...
async function handleFaviconGeneration(request: NextRequest) {
    try {
        ...
        const options = {
            backgroundColor: (formData.get('backgroundColor') as string) || 'transparent',
            ...
            sizes: JSON.parse((formData.get('sizes') as string) || '[16,32,48,64,96,128,180,192,512]'),
        ...
        if (metadata.width < 256 || metadata.height < 256) {
            warnings.push('Source image is smaller than recommended (256x256). Consider using a larger image for best quality.');
        }
        ...
        for (const size of validatedOptions.sizes) {
            if (FAVICON_SIZES[size as keyof typeof FAVICON_SIZES]) {
                ...
                try {
                    ...
                    const paddingPixels = Math.round((validatedOptions.padding / 100) * targetSize);
                    ...
                    if (validatedOptions.format === 'png') {
                        sharpInstance = sharpInstance.png({ quality: Math.round(validatedOptions.quality * 100) });
                    } else if (validatedOptions.format === 'webp') {
                        sharpInstance = sharpInstance.webp({ quality: Math.round(validatedOptions.quality * 100) });
                    } else if (validatedOptions.format === 'jpeg') {
                        sharpInstance = sharpInstance.jpeg({ quality: Math.round(validatedOptions.quality * 100) });
                    }
        ...
        if (validatedOptions.includeManifest) {
            const manifest = {
                ...
                theme_color: validatedOptions.backgroundColor === 'transparent' ? '#ffffff' : validatedOptions.backgroundColor,
                background_color: validatedOptions.backgroundColor === 'transparent' ? '#ffffff' : validatedOptions.backgroundColor,
                display: "standalone"
            };
        ...
        const response = new NextResponse(zipBuffer, {
            ...
            headers: {
                ...
                'Content-Disposition': `attachment; filename="toolchest_favicons_${Date.now()}.zip"`,
 ...
 */
function generateUsageInstructions(favicons: GeneratedFavicon[], hasManifest: boolean): string {
    const icoFavicon = favicons.find(f => f.name === 'favicon.ico');
    const appleTouchIcon = favicons.find(f => f.name === 'apple-touch-icon.png');
    const androidIcons = favicons.filter(f => f.name.includes('android-chrome'));
...
<body>
    ...
    <p><em>Generated by ToolChest Favicon Generator</em></p>
</body>
</html>`;
}
...
export async function GET() {
    return NextResponse.json({
        ...
        features: ['Server-side image processing', 'Multiple formats', 'ZIP packaging', 'Usage instructions']
    });
} 