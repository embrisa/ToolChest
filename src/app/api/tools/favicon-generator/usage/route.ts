...
export async function POST(request: NextRequest) {
  try {
    ...
    const anonymizedData = {
      ...
      // Processing metrics
      processingTimeCategory: categorizeProcessingTime(
        usageData.processingTime,
      ),
      ...
      performanceCategory: usageData.performanceMetrics
        ? categorizePerformance(usageData.performanceMetrics)
        : undefined,

      // Compression metrics (if provided)
      compressionEfficiency: usageData.compressionStats
        ? categorizeCompressionRatio(
            usageData.compressionStats.compressionRatio,
          )
        : undefined,
      ...
      hour:
        new Date(usageData.timestamp).toISOString().slice(0, 13) +
        ":00:00.000Z",

      // Basic technical info (anonymized)
      browserType: usageData.userAgent
        ? categorizeBrowser(usageData.userAgent)
        : "unknown",
    };
...
function categorizePerformance(metrics: {
...
function categorizeWarning(warning: string): string {
  if (warning.includes("large") || warning.includes("size"))
    return "size-warning";
  if (warning.includes("quality") || warning.includes("resolution"))
    return "quality-warning";
  if (warning.includes("format") || warning.includes("type"))
    return "format-warning";
  if (warning.includes("memory") || warning.includes("performance"))
