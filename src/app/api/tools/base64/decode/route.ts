...
export async function POST(request: NextRequest) {
    try {
        ...
        return NextResponse.json(
            {
                success: false,
                error: 'Unsupported content type. Use application/json for text or multipart/form-data for files.'
            },
            { status: 400 }
        );
    } catch (error) {
...
async function handleTextDecoding(request: NextRequest) {
    try {
        ...
        try {
            ...
            if (decodeError instanceof TypeError && decodeError.message.includes('Invalid character')) {
                return NextResponse.json(
                    {
                        success: false,
                        error: 'Invalid UTF-8 sequence. Data may be binary - try file output mode.'
                    },
                    { status: 400 }
                );
            }
...
async function handleFileDecoding(request: NextRequest) {
    try {
        ...
        return NextResponse.json({
            ...
            warnings: file.size > 5 * 1024 * 1024 ? ['Large file processed on server'] : undefined
        });
