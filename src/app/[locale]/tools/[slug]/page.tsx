...
import { getToolComponent, isValidToolSlug } from "@/utils/tools/componentMapper";
...
export default async function ToolPage({
    ...
    const getToolSpecificProps = () => {
        switch (slug) {
            case "base64":
                const keyFeatures = tInfo.raw("keyFeatures.items").map((text: string) => ({ text }));
                const securityAndPrivacy = tInfo.raw("securityAndPrivacy.items").map((text: string) => ({ text }));
                    ...
                    infoSection: {
                        ...
                        sections: [
                            ...
                            },
                            {
                                ...
                                className: "surface p-6 rounded-xl border border-success-200/50 dark:border-success-800/50 bg-success-50/50 dark:bg-success-950/20",
                        ...
                        sections: [
                            {
                                ...
                                items: tInfo.raw("supportedAlgorithms.items").map((text: string) => ({ text })),
                            },
                            {
                                title: tInfo("useCases.title"),
                                titleIcon: { color: "bg-brand-500" },
                                items: tInfo.raw("useCases.items").map((text: string) => ({ text })),
                            },
                        ],
                    },
                };
                        ...
                        sections: [
                            {
                                ...
                                titleIcon: { color: "bg-success-500" },
                                items: tInfo.raw("supportedSizes.items").map((text: string) => ({ text })),
                            },
                            {
                                title: tInfo("outputFormats.title"),
                                titleIcon: { color: "bg-brand-500" },
                                items: tInfo.raw("outputFormats.items").map((text: string) => ({ text })),
                            },
                        ],
                    },
                };
            ...
            case "markdown-to-pdf":
                const features = tInfo.raw("features.items").map((text: string) => ({ text }));
                return {
                    iconText: "📄",
                    ...
                    infoSection: {
                        ...
                        sections: [
                            ...
                            },
                            {
                                ...
                                items: tInfo.raw("supportedFeatures.items").map((text: string) => ({ text })),
                                titleIcon: { color: "bg-brand-500" },
                            },
                            {
                                title: tInfo("pdfTemplates.title"),
                                items: tInfo.raw("pdfTemplates.items").map((text: string) => ({ text })),
                                titleIcon: { color: "bg-accent-500" },
                            },
                            {
    ...
    return (
        <ToolPageTemplate
            ...
            {...toolProps}
        >
            <Suspense fallback={<SuspenseFallback variant="card" message={`Loading ${toolData.name}...`} />}>
                <ToolComponent />
            </Suspense>
        </ToolPageTemplate>
    );
