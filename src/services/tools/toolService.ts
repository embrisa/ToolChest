...
import { PrismaToolWithRelations, PrismaTagWithToolCount } from '@/types/tools/tool';
...
export interface IToolService {
    ...
    getAllToolsPaginated(options: PaginationOptions): Promise<PaginatedResult<ToolDTO>>;
}
...
export class ToolService extends BaseService implements IToolService {
    ...
    async getPopularTools(limit: number): Promise<ToolDTO[]> {
        ...
        return this.getCached(cacheKey, async () => {
            ...
            if (!orderedToolIdResults || orderedToolIdResults.length === 0) {
                ...
                return fallbackTools.map((tool: PrismaToolWithRelations) => toToolDTO(tool));
            }
            ...
            return sortedPopularTools.map((tool: PrismaToolWithRelations) => toToolDTO(tool));
        });
    }
    ...
    async searchTools(query: string): Promise<ToolDTO[]> {
        ...
        return this.getCached(cacheKey, async () => {
            ...
            return searchResults.map((tool: PrismaToolWithRelations) => toToolDTO(tool));
        });
    }
    ...
    async getAllToolsPaginated(options: PaginationOptions): Promise<PaginatedResult<ToolDTO>> {
        const { limit, offset, sortBy = 'displayOrder', sortOrder = 'asc' } = options;
        ...
        return this.getCached(cacheKey, async () => {
            ...
            if (sortBy === 'usageCount') {
                ...
                const toolsMap = new Map(tools.map((tool: PrismaToolWithRelations) => [tool.id, tool]));
                const sortedTools = orderedToolIds
                    .map(id => toolsMap.get(id))
                    .filter(Boolean) as PrismaToolWithRelations[];

                return {
                    tools: sortedTools.map((tool: PrismaToolWithRelations) => toToolDTO(tool)),
                    total: totalCount
                };
            } else {
