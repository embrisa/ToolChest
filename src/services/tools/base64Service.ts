...
export class Base64Service {
    ...
    public static async encode(options: Base64Options): Promise<Base64Result> {
        ...
        try {
            if (options.inputType === 'text') {
                return this.encodeText(options.input as string, options.variant, startTime);
            } else {
    ...
    public static async decode(options: Base64Options): Promise<Base64Result> {
        ...
        try {
            if (options.inputType === 'text') {
                return this.decodeText(options.input as string, options.variant, startTime);
            } else {
     ...
     */
    private static encodeText(text: string, variant: 'standard' | 'url-safe', startTime: number): Base64Result {
        if (!text) {
            return {
                success: false,
        ...
        if (/[\u0080-\uFFFF]/.test(text)) {
            warnings.push('Text contains non-ASCII characters which will be UTF-8 encoded');
        }
     ...
     */
    private static decodeText(base64: string, variant: 'standard' | 'url-safe', startTime: number): Base64Result {
        if (!base64) {
            return {
                success: false,
        ...
        try {
            ...
            if (!/^[A-Za-z0-9+/]*={0,2}$/.test(cleanBase64)) {
                return {
                    success: false,
                    error: 'Invalid Base64 format. Please check your input for invalid characters.',
                    processingTime: Date.now() - startTime
                };
            }
            ...
            if (!this.isValidDecodedText(text)) {
                warnings.push('Decoded data may contain binary content or invalid UTF-8 sequences');
            }
            ...
            return {
                success: false,
                error: 'Failed to decode Base64. The data may be corrupted or not valid Base64.',
                processingTime: Date.now() - startTime
            };
        }
    }
    ...
    public static validateFile(file: File): FileValidationResult {
        ...
        const isAllowedType = this.MIME_CONFIG.allowedTypes.includes(mimeType) ||
        ...
        if (isLargeFile) {
            warnings.push(`Large file (${this.formatBytes(file.size)}) - processing may take longer`);
        }
    ...
    public static async copyToClipboard(text: string): Promise<ClipboardResult> {
        try {
            ...
            return {
                ...
                announceToScreenReader: 'Content has been copied to clipboard successfully'
            };
        } catch {
            ...
            try {
                ...
                if (result) {
                    return {
                        ...
                        method: 'fallback',
                        message: 'Content copied to clipboard',
                        announceToScreenReader: 'Content has been copied to clipboard using fallback method'
                    };
                } else {
                ...
                return {
                    ...
                    announceToScreenReader: 'Copy to clipboard failed. Please select and copy the content manually.'
                };
            }
        }
    }
     ...
     */
    public static generateFilename(mode: 'encode' | 'decode', originalFilename?: string): string {
        ...
        if (mode === 'encode') {
            const baseName = originalFilename ?
                ...
                'encoded';
            return `toolchest_base64_${baseName}_${timestamp}.txt`;
        } else {
            const baseName = originalFilename ?
                originalFilename.replace(/\.[^/.]+$/, '') :
                'decoded';
