...
export class MarkdownToPdfService {
    ...
    constructor() {
        ...
        hljs.configure({
            ...
            languages: [
                ...
                'bash', 'shell', 'json', 'xml', 'html', 'css', 'sql',
                'go', 'rust', 'php', 'markdown', 'yaml', 'dockerfile',
                'kotlin', 'swift', 'ruby', 'perl', 'scala', 'dart',
                'lua', 'r', 'matlab', 'powershell', 'vim', 'makefile'
            ]
        });
    ...
    public parseMarkdown(
        ...
        try {
            ...
            return {
                ...
                tables,
                codeBlocks,
                warnings: isLargeDocument ? ['Large document processed in chunks for optimal performance'] : undefined,
            };
        } catch (error) {
     ...
     */
    private parseMarkdownInChunks(content: string, _options: MarkdownOptions): string {
        const chunks: string[] = [];
        let currentPosition = 0;

        while (currentPosition < content.length) {
            // Find a good breaking point (end of line or paragraph)
            let chunkEnd = Math.min(currentPosition + this.CHUNK_SIZE, content.length);
        ...
        try {
            ...
            try {
                ...
                if (isLargeDocument) {
                    ...
                    pdf = await this.generatePdfInChunks(container, stylingOptions, onProgress);
                } else {
     ...
     */
    public async validateMarkdownFile(file: File): Promise<MarkdownFileValidation> {
        ...
        if (!hasValidExtension) {
            ...
            validation.warnings.push('File extension is not a common markdown format. Supported: .md, .markdown, .txt');
        }
        ...
        try {
            ...
            if (content.includes('�')) {
                validation.warnings = validation.warnings || [];
                validation.warnings.push('File may have encoding issues. Some characters may not display correctly.');
            }
            ...
            validation.isValid = false;
            validation.error = 'Unable to read file content. File may be corrupted or in an unsupported format.';
        }
    ...
    public createDownloadOptions(
        ...
        const defaultFilename = `toolchest_markdown_${timestamp}.pdf`;
    ...
    private generatePdfCss(options: PdfStylingOptions): string {
        ...
        const enhancedFontSize = this.getAccessibilityFontSize(fontSize, accessibility?.fontSize);

        // Get high contrast colors if enabled
        const colors = this.getAccessibilityColors(backgroundColor, textColor, linkColor, accessibility?.highContrast);

        const syntaxCss = syntaxHighlighting?.enabled ? this.getSyntaxHighlightingCss(syntaxHighlighting.theme) : '';
     ...
     */
    private createPdfFromCanvas(canvas: HTMLCanvasElement, options: PdfStylingOptions): jsPDF {
        const { format, orientation } = options;
        const dimensions = PDF_FORMAT_DIMENSIONS[format];

        const pdfWidth = orientation === 'landscape' ? dimensions.height : dimensions.width;
        const pdfHeight = orientation === 'landscape' ? dimensions.width : dimensions.height;
        ...
        const availableWidth = pdfWidth - options.margin.left - options.margin.right;
        const availableHeight = pdfHeight - options.margin.top - options.margin.bottom;
     ...
     */
    private generatePdfMetadata(options: PdfStylingOptions, html: string): PdfMetadata {
        return {
            title: 'Markdown Document',
            creator: 'ToolChest Markdown-to-PDF',
            producer: 'ToolChest',
     ...
     * Get accessibility-enhanced font size
     */
    private getAccessibilityFontSize(baseFontSize: number, accessibilityFontSize?: 'small' | 'normal' | 'large' | 'x-large'): number {
