...
export interface IRelationshipService {
    ...
    previewBulkOperation(operation: BulkTagOperation): Promise<BulkOperationPreview>;

    executeBulkOperation(operation: BulkTagOperation): Promise<BulkOperationResult>;
    ...
    bulkAssignTags(toolIds: string[], tagIds: string[]): Promise<BulkOperationResult>;

    bulkRemoveTags(toolIds: string[], tagIds: string[]): Promise<BulkOperationResult>;
...
export class RelationshipService extends BaseService implements IRelationshipService {
    ...
    async getAllRelationships(
        filters: RelationshipFilters = {},
        sortOptions: RelationshipSortOptions = { field: 'toolName', direction: 'asc' }
    ): Promise<ToolTagRelationship[]> {
    ...
    async getTagUsageStatistics(tagId?: string): Promise<TagUsageStatistics[]> {
        ...
        return this.getCached(cacheKey, async () => {
            ...
            const statistics = await Promise.all(tags.map(async (tag, index) => {
                const activeTools = tag.tools.filter(t => t.tool.isActive).length;
                const inactiveTools = tag.tools.filter(t => !t.tool.isActive).length;
                const usagePercentage = totalTools > 0 ? (tag._count.tools / totalTools) * 100 : 0;
                ...
                return {
                    ...
                    recentUsage: {
                        lastUsed: tag.tools.length > 0 ? tag.tools[0]?.tool ? new Date() : null : null,
    ...
    async previewBulkOperation(operation: BulkTagOperation): Promise<BulkOperationPreview> {
        ...
        const toolsToUpdate = tools.map(tool => {
            ...
            if (operation.type === 'assign') {
                ...
                newTagIds = currentTagIds.filter(id => !operation.tagIds.includes(id));
            }
        ...
        const summary = {
            ...
            totalTagChanges: toolsToUpdate.reduce((sum, tool) =>
                sum + tool.addedTags.length + tool.removedTags.length, 0
            ),
            newRelationships: toolsToUpdate.reduce((sum, tool) => sum + tool.addedTags.length, 0),
            removedRelationships: toolsToUpdate.reduce((sum, tool) => sum + tool.removedTags.length, 0)
        };
        ...
        if (operation.type === 'remove') {
            const toolsBecomingUntagged = toolsToUpdate.filter(tool => tool.newTags.length === 0);
            if (toolsBecomingUntagged.length > 0) {
                warnings.push(`${toolsBecomingUntagged.length} tool(s) will have no tags after this operation`);
    ...
    async executeBulkOperation(operation: BulkTagOperation): Promise<BulkOperationResult> {
        const errors: string[] = [];
        const warnings: string[] = [];
        let totalChanges = 0;
        let toolsAffected = 0;
        ...
        try {
            ...
            if (preview.requiresConfirmation && !operation.requiresConfirmation) {
                throw new Error('This operation requires confirmation due to potential data impact');
            }

            // Execute the operation
            for (const toolUpdate of preview.toolsToUpdate) {
                if (toolUpdate.addedTags.length === 0 && toolUpdate.removedTags.length === 0) {
                ...
                try {
                    await this.prisma.tool.update({
                        ...
                        data: {
                            tags: {
                                deleteMany: operation.type === 'assign' ? {} : {
                                    tagId: { in: toolUpdate.removedTags }
                                },
                                create: operation.type === 'assign' ?
                                    toolUpdate.addedTags.map(tagId => ({ tagId })) : []
                            }
                        }
                    });

                    totalChanges += toolUpdate.addedTags.length + toolUpdate.removedTags.length;
                    toolsAffected++;
                } catch (err) {
                    errors.push(`Failed to update tool ${toolUpdate.name}: ${err instanceof Error ? err.message : 'Unknown error'}`);
                }
            }
    ...
    async bulkAssignTags(toolIds: string[], tagIds: string[]): Promise<BulkOperationResult> {
    ...
    async bulkRemoveTags(toolIds: string[], tagIds: string[]): Promise<BulkOperationResult> {
        const operation: BulkTagOperation = {
            type: 'remove',
            toolIds,
            tagIds,
    ...
    async findOrphanedEntities(): Promise<OrphanedEntityCheck> {
        ...
        if (orphanedTools.length > 0) {
            suggestedActions.push(`Assign tags to ${orphanedTools.length} untagged tool(s)`);
        }

        if (orphanedTags.length > 0) {
            suggestedActions.push(`Review ${orphanedTags.length} unused tag(s) for removal or assignment`);
