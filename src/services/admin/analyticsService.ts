...
export class AnalyticsService extends BaseService {
    ...
    private async checkAlertThresholds(metrics: RealTimeMetrics): Promise<void> {
        ...
        for (const check of checks) {
            if (check.value > check.threshold) {
                await this.triggerAlert(check.type, check.title, check.description, check.threshold, check.value);
            }
        }
    }
    ...
    private async triggerAlert(
        ...
        if (!existingAlert) {
            const severity = this.calculateAlertSeverity(type, threshold, currentValue);
    ...
    private async performHealthChecks(): Promise<SystemHealthCheck[]> {
        ...
        checks.push({
            service: 'Memory',
            status: memoryPercentage < 70 ? 'healthy' : memoryPercentage < 85 ? 'degraded' : 'unhealthy',
            responseTime: 0,
            lastCheck: new Date(),
            details: {
     ...
     */
    private calculateOverallHealth(checks: SystemHealthCheck[]): 'healthy' | 'degraded' | 'unhealthy' {
     ...
     */
    public async getErrorLogs(filter?: MonitoringFilter): Promise<ErrorLogEntry[]> {
        ...
        if (filter) {
            ...
            if (filter.errorLevels?.length) {
                filteredLogs = filteredLogs.filter(log => filter.errorLevels!.includes(log.level));
            }
     ...
     */
    public async acknowledgeAlert(alertId: string, acknowledgedBy: string = 'admin'): Promise<void> {
     ...
     */
    public async getRealTimeMetrics(limit: number = 100): Promise<RealTimeMetrics[]> {
    ...
    private scheduleCleanup(): void {
        const cleanup = () => {
            ...
            this.activeAlerts = this.activeAlerts.filter(
                alert => !alert.resolved || (alert.resolved && alert.resolved > alertCutoff)
            );

            // Clean up old metrics (keep last 24 hours)
            const metricsCutoff = new Date(Date.now() - 24 * 60 * 60 * 1000);
            this.metricsHistory = this.metricsHistory.filter(metric => metric.timestamp > metricsCutoff);
        };
        ...
        try {
            ...
            return await this.getCached(cacheKey, async () => {
                ...
                const [totalTools, totalTags, totalUsage] = await Promise.all([
                    this.prisma.tool.count({ where: { isActive: !filter?.includeInactive ? true : undefined } }),
 ...
 */
    public async getToolUsageAnalytics(filter?: AnalyticsFilter): Promise<ToolUsageAnalytics[]> {
        try {
            const cacheKey = `tool_usage_analytics_${JSON.stringify(filter)}`;

            return await this.getCached(cacheKey, async () => {
                const timeRange = filter?.timeRange || this.getDefaultTimeRange();
                const toolFilter = filter?.toolIds ? { id: { in: filter.toolIds } } : {};
                ...
                const analytics: ToolUsageAnalytics[] = await Promise.all(
                    tools.map(async (tool) => {
                        ...
                        return {
                            ...
                            averageSessionTime: await this.getAverageSessionTime(tool.id, timeRange),
 ...
 */
    public async generateCharts(filter?: AnalyticsFilter): Promise<AnalyticsChart[]> {
        try {
            const timeRange = filter?.timeRange || this.getDefaultTimeRange();
     ...
     */
    public async exportAnalytics(options: ExportOptions, filter?: AnalyticsFilter): Promise<AnalyticsExport> {
        try {
    ...
    private async getTotalUsageCount(timeRange: AnalyticsTimeRange): Promise<number> {
    ...
    private async getActiveUsersCount(timeRange: AnalyticsTimeRange): Promise<number> {
    ...
    private async getRecentActivity(timeRange: AnalyticsTimeRange, limit: number) {
    ...
    private async getPeriodComparison(timeRange: AnalyticsTimeRange) {
        ...
        const [currentUsage, previousUsage] = await Promise.all([
            this.getTotalUsageCount(timeRange),
            this.getTotalUsageCount({ start: previousStart, end: previousEnd, period: timeRange.period }),
        ]);
        ...
        return {
            ...
            growthRates: {
                usage: previousUsage > 0 ? ((currentUsage - previousUsage) / previousUsage) * 100 : 0,
                newUsers: previousUsers > 0 ? ((currentUsers - previousUsers) / previousUsers) * 100 : 0,
    ...
    private groupUsagesByPeriod(usages: any[], period: 'day' | 'week' | 'month'): number[] {
    ...
    private calculateGrowthRates(trend: { daily: number[]; weekly: number[]; monthly: number[] }) {
    ...
    private async getUniqueUsersCount(toolId: string, timeRange: AnalyticsTimeRange): Promise<number> {
    ...
    private async getAverageSessionTime(_toolId: string, _timeRange: AnalyticsTimeRange): Promise<number> {
        // Return mock data - in real implementation, this would track session duration
        return 180; // 3 minutes average
    }

    private async generateUsageChart(timeRange: AnalyticsTimeRange): Promise<AnalyticsChart> {
        const usages = await this.prisma.toolUsage.findMany({
            where: {
                timestamp: {
                    gte: timeRange.start,
    ...
    private async generateTopToolsChart(timeRange: AnalyticsTimeRange): Promise<AnalyticsChart> {
    ...
    private async generateTrendsChart(timeRange: AnalyticsTimeRange): Promise<AnalyticsChart> {
        ...
        tools.forEach((tool) => {
            ...
            categoryUsage[category] = (categoryUsage[category] || 0) + tool.usages.length;
        });
    ...
    private generateDateLabels(timeRange: AnalyticsTimeRange, period: 'day' | 'week' | 'month'): string[] {
        const labels: string[] = [];
