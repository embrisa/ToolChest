...
export interface IAdminTagService {
    ...
    checkDeleteRelationshipWarnings(id: string): Promise<AdminTagRelationshipWarning | null>;
}
...
export class AdminTagService extends BaseService implements IAdminTagService {
    ...
    async createTag(data: AdminTagCreateRequest): Promise<TagDTO> {
        // Validate the data
        const validationErrors = await this.validateTagData(data as AdminTagFormData);
    ...
    async validateTagData(data: AdminTagFormData): Promise<AdminTagValidationErrors> {
    ...
    async checkDeleteRelationshipWarnings(id: string): Promise<AdminTagRelationshipWarning | null> {
        const tag = await this.prisma.tag.findUnique({
            where: { id },
            include: {
                tools: {
