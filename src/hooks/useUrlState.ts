...
export function useUrlState(options: UseUrlStateOptions = {}) {
    ...
    }, [pathname, router, searchParams, replace]);

    // Get a specific parameter value
    const getParam = useCallback((key: string): string | string[] | undefined => {
        return urlState[key];
    }, [urlState]);

    // Set a specific parameter value
    const setParam = useCallback((key: string, value: string | string[] | null | undefined) => {
    ...
    const updateParams = useCallback((updates: Record<string, string | string[] | null | undefined>) => {
        setUrlState(updates);
    }, [setUrlState]);
...
export function useToolFilterState() {
    ...
    const { urlState, updateParams, clearParams } = useUrlState({
        defaultValues,
