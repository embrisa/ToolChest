...
export function usePerformanceOptimization() {
    ...
    const cacheStats = useRef<Map<string, { hits: number; misses: number }>>(new Map());
                ...
                entries.forEach((entry) => {
                    ...
                    if (entry.entryType === 'event') {
                        ...
                        if (eventEntry.name === 'click') {
                            setMetrics(prev => ({
                                ...prev,
                                interactionTime: eventEntry.processingStart - eventEntry.startTime,
                            }));
                        }
                    }
                });
    ...
    const preloadResource = useCallback(async (
        ...
        const attemptPreload = async (attempt: number): Promise<void> => {
            try {
                ...
                console.warn(`Failed to preload ${url} after ${retries} attempts:`, error);
            }
        };
    ...
    const preloadCriticalResources = useCallback(async () => {
        ...
        const preloadPromises = criticalResources.map(resource =>
            preloadResource(resource.url, resource.type, { priority: resource.priority })
        );
    ...
    const getCacheStats = useCallback(() => {
        const stats = Array.from(cacheStats.current.entries()).map(([key, data]) => ({
            key,
            hits: data.hits,
            misses: data.misses,
        ...
        return {
            ...
            overallHitRate: metrics.cacheHits / (metrics.cacheHits + metrics.cacheMisses) || 0,
        };
    }, [metrics]);
