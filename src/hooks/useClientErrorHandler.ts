...
// State reducer
function errorStateReducer(state: ClientErrorState, action: ErrorAction): ClientErrorState {
    switch (action.type) {
        case 'ADD_ERROR':
            return {
                ...state,
        ...
        case 'REMOVE_ERROR':
            return {
                ...state,
                errors: state.errors.filter(error => error.requestId !== action.payload),
            };
        case 'CLEAR_ERRORS':
            return {
                ...state,
...
export function useClientErrorHandler() {
    ...
    const getEnhancedContext = useCallback((context?: Record<string, any>): EnhancedErrorContext => {
        const performanceInfo = typeof window !== 'undefined' && 'performance' in window ? {
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                const timeoutId = setTimeout(async () => {
                    try {
                        const result = await retryWithBackoff(operation, errorId, attempt + 1, maxRetries, baseDelay);
                        resolve(result);
                    } catch (retryError) {
                        reject(retryError);
                    }
        ...
        try {
            switch (strategy) {
                ...
                case 'retry_with_backoff':
                    if (operation) {
                        const attempts = recoveryAttemptsRef.current.get(error.requestId || '') || 0;
                ...
                case 'report':
                    ...
                    const reportNotification = createCriticalToast(
                        ...
                        [{
                            ...
                            action: () => {
                                const subject = encodeURIComponent(`Error Report - ${error.requestId || 'Unknown'}`);
            ...
            if (config.fallbackStrategy && config.fallbackStrategy !== strategy) {
                return await executeRecoveryStrategy(config.fallbackStrategy, error, config, operation);
            }
    ...
    const handleError = useCallback(async (
        ...
        if (!silent && recoveryConfig.showUserNotification) {
            ...
            } else if (category === 'server' || clientError.severity === 'critical') {
                notification = createCriticalToast(
                    'Critical Error',
                    friendlyMessage,
                    [{
                        label: 'Report Error',
                        action: () => executeRecoveryStrategy('report', clientError, recoveryConfig),
                        variant: 'primary'
                    }, {
                        label: 'Refresh Page',
                        action: () => executeRecoveryStrategy('reload', clientError, recoveryConfig),
                        variant: 'secondary'
                    }]
                );
            } else {
        ...
        if (isRecoverableError(error)) {
            ...
            try {
                await executeRecoveryStrategy(recoveryConfig.strategy, clientError, recoveryConfig);
            } catch (recoveryError) {
    ...
    return {
        ...
        clearNotifications,
        clearErrors,
        executeRecoveryStrategy: (strategy: ErrorRecoveryStrategy, error: ClientError, config?: Partial<ErrorRecoveryConfig>) =>
            executeRecoveryStrategy(strategy, error, { ...defaultRecoveryConfig, ...config }),
    };
} 