...
import { LoadingManager, LoadingStateConfig, LoadingAccessibility } from '@/types/ui/loading';

export function useLoadingManager(): LoadingManager & LoadingAccessibility {
    const [loadingStates, setLoadingStates] = useState<Record<string, boolean>>({});
    const [loadingConfigs, setLoadingConfigs] = useState<Record<string, LoadingStateConfig>>({});
    ...
    const setProgressAnnouncement = useCallback((progress: number, total?: number) => {
...
export function useGlobalLoading() {
    ...
    const updateGlobalProgress = useCallback((progress: number, message?: string) => {
