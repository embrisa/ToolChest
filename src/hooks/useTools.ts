...
export function useTools() {
    const { data, error, isLoading, mutate } = useSWR<ToolDTO[]>('/tools', swrFetcher);
...
export function useToolsSearch(query: string) {
    ...
    const endpoint = shouldFetch ? `/tools/search?q=${encodeURIComponent(query)}` : null;
...
export function useTags() {
    const { data, error, isLoading, mutate } = useSWR<TagDTO[]>('/tags', swrFetcher);
...
export function useToolsByTag(tagSlug: string | null) {
    ...
    const endpoint = shouldFetch ? `/tools?tag=${encodeURIComponent(tagSlug)}` : null;
