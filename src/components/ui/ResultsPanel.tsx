    ...
    <Card variant="elevated" className={cn("animate-fade-in-up", className)}>
              ...
              >
                {metadata.map((item, index) => (
                  <span
                    ...
                    className={cn(
                      variant === "file" &&
                        "bg-neutral-50 dark:bg-neutral-900/20 px-3 py-1 rounded-lg",
                    )}
                  >
          ...
          {hasActions && (
            <div
              className={cn(
                "flex gap-3",
                variant === "file" &&
                  "mt-4 sm:mt-0 w-full sm:w-auto justify-center sm:justify-end",
              )}
            >
              {onCopy && (
                <Button
      ...
      {(shouldShowTextarea || children) && (
        <CardContent className="pt-0">
          <div className={cn("space-y-6", variant === "file" && "space-y-4")}>
            {shouldShowTextarea && (
              <textarea
                value={result || ""}
                readOnly={readOnly}
