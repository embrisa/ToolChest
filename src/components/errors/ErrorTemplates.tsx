...
export function HttpErrorPage({
    ...
    return (
        <ErrorPage
                ...
                onClick: () => {
                    switch (action.type) {
                        ...
                        case 'contact_support':
                            const subject = encodeURIComponent(`Support Request - Error ${statusCode}`);
            ...
            showTechnicalDetails={showTechnicalDetails || config.showTechnicalDetails}
            technicalDetails={technicalDetails}
        />
    );
}
...
export function NotFoundErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
    return <HttpErrorPage statusCode={404} {...props} />;
}

export function UnauthorizedErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
    return <HttpErrorPage statusCode={401} {...props} />;
}

export function ForbiddenErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
    return <HttpErrorPage statusCode={403} {...props} />;
}

export function RateLimitErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
    return <HttpErrorPage statusCode={429} {...props} />;
}

export function InternalServerErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
    return <HttpErrorPage statusCode={500} {...props} />;
}

export function BadGatewayErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
    return <HttpErrorPage statusCode={502} {...props} />;
}

export function ServiceUnavailableErrorPage(props?: Omit<HttpErrorPageProps, 'statusCode'>) {
...
export function MaintenanceErrorPage({ estimatedDuration }: { estimatedDuration?: string } = {}) {
    return (
        <ErrorPage
            statusCode={503}
