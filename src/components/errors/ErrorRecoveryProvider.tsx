...
interface ErrorRecoveryContextType {
    handleError: (error: Error, options?: ClientErrorHandlerOptions) => Promise<void>;
...
export function withErrorRecovery<P extends object>(
    ...
    const WithErrorRecoveryComponent = (props: P) => {
        ...
        const handleComponentError = (error: Error, errorInfo: any) => {
            handleError(error, {
                component: WrappedComponent.displayName || WrappedComponent.name || 'Component',
                context: {
                    props,
                    errorInfo,
                },
        ...
        return (
            <ErrorBoundary
                fallback={FallbackComponent ?
                    <FallbackComponent error={new Error('Component error')} retry={retryRender} /> :
                    undefined
                }
    ...
    WithErrorRecoveryComponent.displayName = `withErrorRecovery(${WrappedComponent.displayName || WrappedComponent.name || 'Component'
        })`;
